package com.gugu42.rcmod.common.world;
...
public class TntCrateExplosion extends Explosion {
    ...
    @OnlyIn(Dist.CLIENT)
    public TntCrateExplosion(World worldIn, @Nullable Entity entityIn, double x, double y, double z, float size, List<BlockPos> affectedPositions) {
        this(worldIn, entityIn, x, y, z, size, false, Explosion.Mode.DESTROY, affectedPositions);
    }

    @OnlyIn(Dist.CLIENT)
    public TntCrateExplosion(World worldIn, @Nullable Entity exploderIn, double xIn, double yIn, double zIn, float sizeIn, boolean causesFireIn, Explosion.Mode modeIn, List<BlockPos> affectedBlockPositionsIn) {
    ...
    @OnlyIn(Dist.CLIENT)
    public TntCrateExplosion(World worldIn, @Nullable Entity exploderIn, double xIn, double yIn, double zIn, float sizeIn, boolean causesFireIn, Explosion.Mode modeIn) {
        this(worldIn, exploderIn, (DamageSource) null, (ExplosionContext) null, xIn, yIn, zIn, sizeIn, causesFireIn, modeIn);
    }

    public TntCrateExplosion(World world, @Nullable Entity exploder, @Nullable DamageSource source, @Nullable ExplosionContext context, double x, double y, double z, float size, boolean causesFire, Explosion.Mode mode) {
                ...
                for (int l = 0; l < 16; ++l) {
                    if (j == 0 || j == 15 || k == 0 || k == 15 || l == 0 || l == 15) {
                        ...
                        for (float f1 = 0.3F; f > 0.0F; f -= 0.22500001F) {
                            ...
                            if (f > 0.0F &&
                                    (blockstate.getBlock() == RcBlocks.CRATE_BLOCK.get()
                                            || blockstate.getBlock() == RcBlocks.AMMO_CRATE_BLOCK.get())) {
                                set.add(blockpos);
                            }
