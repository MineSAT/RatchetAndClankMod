...
public class TNTCrateExplosion extends Explosion {
    ...
    public TNTCrateExplosion(World worldIn, Entity entityIn, double x, double y, double z, float size, List<BlockPos> affectedPositions)
    {
        this(worldIn, entityIn, x, y, z, size, false, true, affectedPositions);
    }

    @SideOnly(Side.CLIENT)
    public TNTCrateExplosion(World worldIn, Entity entityIn, double x, double y, double z, float size, boolean flaming, boolean smoking, List<BlockPos> affectedPositions)
    {
        this(worldIn, entityIn, x, y, z, size, flaming, smoking);
        this.affectedBlockPositions.addAll(affectedPositions);
    }

    public TNTCrateExplosion(World worldIn, Entity entityIn, double x, double y, double z, float size)
    {
    	this(worldIn, entityIn, x, y, z, size, false, false);
    }
    
    public TNTCrateExplosion(World worldIn, Entity entityIn, double x, double y, double z, float size, boolean flaming, boolean smoking)
    {
                ...
                for (int l = 0; l < 16; ++l)
                {
                    if (j == 0 || j == 15 || k == 0 || k == 15 || l == 0 || l == 15)
                    {
                        ...
                        for (float f1 = 0.3F; f > 0.0F; f -= 0.22500001F)
                        {
                            ...
                            {
                                float f2 = this.exploder != null ? this.exploder.getExplosionResistance((Explosion)this, this.world, blockpos, iblockstate) : iblockstate.getBlock().getExplosionResistance(world, blockpos, (Entity)null, this);
                                f -= (f2 + 0.3F) * 0.3F;
                            }
