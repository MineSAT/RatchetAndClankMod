...
public class EntityRYNOAmmo extends EntityThrowable {
	...
	public void onUpdate() {
		...
		if (this.target != null) {
			double d = this.target.boundingBox.minX + (this.target.boundingBox.maxX - this.target.boundingBox.minX) / 2.0D - this.posX;
			double d1 = this.target.boundingBox.minY + (this.target.boundingBox.maxY - this.target.boundingBox.minY) / 2.0D - this.posY;
			double d2 = this.target.boundingBox.minZ + (this.target.boundingBox.maxZ - this.target.boundingBox.minZ) / 2.0D - this.posZ;
	...
	private EntityLiving getTarget(double d, double d1, double d2, double d3) {
		...
		List list = this.worldObj.getEntitiesWithinAABBExcludingEntity(getThrower(), this.boundingBox.expand(16.0D, 16.0D, 16.0D));
	...
	protected void onImpact(MovingObjectPosition movingobjectposition) {
		...
		if ((!this.isDead) && (movingobjectposition.typeOfHit == MovingObjectType.BLOCK) && (!this.worldObj.isRemote))
			setDead();
	}
	...
	private EntityLiving getNearestEntity() {
		...
		if (!this.worldObj.isRemote) {
			List list = this.worldObj.getEntitiesWithinAABBExcludingEntity(getThrower(), AxisAlignedBB.getBoundingBox(i, k, i2, j, l1, j2));
